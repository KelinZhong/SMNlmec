...
dSL<-function(y,mu,sigma,nu){
  f<-function(u) nu*u^(nu-1)*dmnorm(y,mu,u^(-1)*sigma)
...
acumSL<-function(y,mu,sigma,nu){
  f<-function(u) pmnorm(y,mu,u^(-1)*sigma)*nu*u^(nu-1)
  resp <- integrate(Vectorize(f),0,1)$value
  return(resp)
}
...
grafSL<-function(y0,mu0,sigma0,Qc,muc,sigmac,nu){
  f<-function(u) dmnorm(y0,mu0,u^(-1)*sigma0)*pmnorm(Qc,muc,u^(-1)*sigmac)*nu*u^(nu-1)
  resp <- integrate(Vectorize(f),0,1)$value
  return(resp)
}
  ...
  if(distr=="Normal"){
    ...
    for (j in 1:m){
      ...
      if(sum(cc1)==0){     
        ...
        ver[j]<- dmvnorm(c(y1),mean=c(gammai),sigma=Psi) 
      ...
      if(sum(cc1)>=1){
        ...
        if(sum(cc1)==nj[j]){
          ...
          ver[j]<-pmvnorm(LI1, LS1, mean=c(gammai),sigma=Psi,algorithm = GB) 
          ...
          ver[j]<-dmvnorm(c(y1[cc1==0]),mean=c(gammai[cc1==0]),sigma=So)*(pmvnorm(LI1[cc1==1],LS1[cc1==1],mean=as.vector(muc),sigma=Sc,algorithm = GB))[1]
        }
      }  
    }
  }
  ...
  if(distr=="Student"){
    ...
    for (j in 1:m){
      ...
      if(sum(cc1)==0){     
        ...
        ver[j]<- dtmvt((y1), mean=c(gammai),sigma=Psi,df=nu,lower=rep(-Inf,length(mean)),upper=rep(Inf,length(mean))) 
      ...
      if(sum(cc1)>=1){
        ...
        if(sum(cc1)==nj[j]){
          ...
          ver[j]<-ptmvt(lowerx=(LI1), upperx=(LS1), mean=c(gammai),  sigma=Psi, df=nu)
          ...
          ver[j]<-dmvt((y1[cc1==0]),delta=c(gammai[cc1==0]),sigma=So,df=nu,log=FALSE)*pmvt(lower=LI1[cc1==1], upper=LS1[cc1==1], delta=c(muc), df=nu1, sigma=SigmaUi.1,algorithm = GB)[1]
